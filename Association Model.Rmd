...
We can see that all reported p-values are less than the pre-specified specificity, and thus a crude association can be established.
...
```{r}
100*(summary(lm.bwfull_age)$coefficients[2,1] - summary(lm.par_gestwks)$coefficients[2,1]) / ( summary(lm.par_gestwks)$coefficients[2,1])
```

To compute $\Delta \hat{\beta_2}$:

```{r}
100*(summary(lm.bwfull_age)$coefficients[3,1] - summary(lm.par_gestwks)$coefficients[3,1]) / ( summary(lm.par_gestwks)$coefficients[3,1])
```

To compute $\Delta \hat{\beta_3}$:

```{r}
100*(summary(lm.bwfull_age)$coefficients[4,1] - summary(lm.par_gestwks)$coefficients[4,1]) / ( summary(lm.par_gestwks)$coefficients[4,1])
```
...
```{r}
100*(summary(lm.bwfull_age)$coefficients[2,1] - summary(lm.par_BMI)$coefficients[2,1]) / ( summary(lm.par_BMI)$coefficients[2,1])
```

To compute $\Delta \hat{\beta_2}$:

```{r}
100*(summary(lm.bwfull_age)$coefficients[3,1] - summary(lm.par_BMI)$coefficients[3,1]) / ( summary(lm.par_BMI)$coefficients[3,1])
```

To compute $\Delta \hat{\beta_3}$:

```{r}
100*(summary(lm.bwfull_age)$coefficients[4,1] - summary(lm.par_BMI)$coefficients[4,1]) / ( summary(lm.par_BMI)$coefficients[4,1])
```
...
```{r}
100*(summary(lm.bwfull_age)$coefficients[2,1] - summary(lm.par_mheight)$coefficients[2,1]) / ( summary(lm.par_mheight)$coefficients[2,1])
```

To compute $\Delta \hat{\beta_2}$:

```{r}
100*(summary(lm.bwfull_age)$coefficients[3,1] - summary(lm.par_mheight)$coefficients[3,1]) / ( summary(lm.par_mheight)$coefficients[3,1])
```

To compute $\Delta \hat{\beta_3}$:

```{r}
100*(summary(lm.bwfull_age)$coefficients[4,1] - summary(lm.par_mheight)$coefficients[4,1]) / ( summary(lm.par_mheight)$coefficients[4,1])
```
...
```{r}
fitted <- lm.bwfull_age$fitted.values # extract fitted values
resm <- resid(lm.bwfull_age)          # extract model residuals
standm <- rstandard(lm.bwfull_age)    # extract standardized residuals
studm <- rstudent(lm.bwfull_age)      # extract studentized residuals
...
CHDS$h <- hatvalues(lm.bwfull_age)      # Add leverages to the dataset
CHDS$c <- cooks.distance(lm.bwfull_age) # add Cook's distance to the dataset
```
...
Upon examination of the data reported in the second table, none of the data is wildly implausable, with the exception of a 48 week gestational age.  A 48-week gestational age is well above the 0.1th percentile of gestational age based on ultrasound based estimates of gestation; and furthermore none of the women in this study (n=1867) had gestation age greater than 46 weeks.1
